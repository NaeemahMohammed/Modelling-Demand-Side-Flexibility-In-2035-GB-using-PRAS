...
# 0. PROJECT ROOT
# -----------------------------------------------------------------------------
const ROOT = raw"C:\Users\nam125\OneDrive - Imperial College London\Desktop\Individual Research Project\GBReliability2035"
...
const DISPLAY_WEATHER_YEAR = 2020
println("  Detected from data: N_HOURS = $N_HOURS, simulation year = $WEATHER_YEAR")
println("  Actual weather year: $DISPLAY_WEATHER_YEAR (WY:2020 profiles)")
...
function embed_vres_for!(gen_capacity, gen_categories, indiv_for)
    ...
    for i in eachindex(gen_categories)
        ...
        if haskey(indiv_for, c)
            # Multiply Float capacity by (1-FOR) then round back to Int
...
const INDIV_FOR = Dict(
    "wind_offshore"  => 0.030,   # Carroll et al. (2016): offshore 3.0%
    "wind_onshore"   => 0.014,   # Carroll et al. (2016): onshore  1.4%
    "embedded_wind"  => 0.014,   # Carroll et al. (2016): as onshore
    "solar_pv"       => 0.010,   # IEA PVPS T13-23:       1.0%
    "embedded_solar" => 0.010,   # IEA PVPS T13-23:       as solar_pv
)
...
# Scale factor: ratio of corrected to PyPSA-GB implied discharge capacity
# Applied to H2 turbine line backward capacities in the lines section below
