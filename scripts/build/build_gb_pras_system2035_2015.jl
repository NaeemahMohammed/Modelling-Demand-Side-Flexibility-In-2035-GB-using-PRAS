...
#      bus automatically.
#
#   4. Hydrogen-to-power turbines (PyPSA Links: GB_H2 -> zone) are added as
#      firm Generators, and electrolysis demand (PyPSA Links: zone -> GB_H2)
#      is added as firm load -- neither appears in generators_static.csv or
#      loads_p_set.csv, since PyPSA models both as energy-converter Links.
#      p_nom on a PyPSA Link is rated on the bus0 (input) side, so the
#      turbines' ELECTRICAL capacity is p_nom * efficiency; electrolysis
#      p_nom is already on the electricity side, no correction needed there.
#
#      LIMITATION: electrolysis demand (7,426 MW, ~15% of system peak) is
#      added as constant, always-on firm demand. In reality electrolysers
#      are price-responsive and would likely be curtailed during system
#      stress before firm demand is shed, so this is a conservative
#      (pessimistic) treatment, not a measured dispatch profile -- PyPSA's
#      actual optimised hourly flow on these links isn't available in the
#      static input files used here. Document this in the write-up.
#
...
# 0. PROJECT ROOT
# -----------------------------------------------------------------------------
const ROOT = raw"C:\Users\nam125\OneDrive - Imperial College London\Desktop\Individual Research Project\GBReliability2035_2015"
...
# Override: PyPSA-GB stamps timestamps with simulation year (2035), not the
# historical weather year used for the profiles.
const DISPLAY_WEATHER_YEAR = 2015
println("  Detected from data: N_HOURS = $N_HOURS, simulation year = $WEATHER_YEAR")
println("  Actual weather year: $DISPLAY_WEATHER_YEAR (WY:2015 profiles)")
...
# -----------------------------------------------------------------------------
# 2. REGIONS -- GB zones only, real hourly load, PLUS electrolysis demand
#    (PyPSA models this as a zone -> GB_H2 Link, so it never appears in
#    loads_p_set.csv).
# -----------------------------------------------------------------------------
...
# Electrolysis links (zone -> GB_H2) are handled via the GB_H2 Storage
# object below -- charging the storage when the zone has surplus power.
# They are NOT added as fixed demand here (previous treatment removed).
electrolysis_links = filter(row -> row.bus1 == "GB_H2", links_df)
println("  Found $(nrow(electrolysis_links)) electrolysis link(s) totalling ",
        round(sum(electrolysis_links.p_nom), digits=1), " MW -- modelled via GB_H2 Storage (see section 4)")
...
# GB_H2 hydrogen system is now modelled as a Storage object (section 4).
# No separate generator needed here -- the turbine dispatch happens via
# the Lines (GB_H2 -> zone) and the storage discharge in section 4.
...
# ── Embed individual turbine/panel FOR into gen_capacity for variable RES ─────
# These generators are zone-level fleet aggregations (e.g. 24,225 MW = ~2,400
# turbines). Fleet-level correlated outages are physically impossible (P≈10^-3000).
# We instead scale each hour's capacity by (1 - individual_unit_FOR), correctly
# reflecting that on average (FOR × p_nom) MW is unavailable due to individual
# turbine/panel failures. The MTTF in the reliability CSV is set to 1,000,000h
# to prevent PRAS from ever triggering a full fleet outage.
#
# FORs from: wind — Carroll et al. (2016); solar — IEA PVPS T13-23.
# This is equivalent to Elina's (N-n)/N formulation at the expected-value level:
# for large N, Binomial(N, f) has σ/N ≈ 0.1%, making fixed (1-f) scaling exact.
...
# region_stor_idxs for the 20 GB zones (GB_H2 storage appended in section 5)
...
println("  $(nrow(storage_df)) conventional storage units across $(n_regions - 1) GB zones (GB_H2 storage added in section 5)")
...
#    Full discharge duration: 658,421 / (discharge_cap/discharge_eff) ≈ 168h (7 days)
#    Full charge duration:    658,421 / (charge_cap × charge_eff)     ≈ 127h (5.3 days)
#
#    The H2 turbine Lines (GB_H2 -> zone) in section 6 carry the discharged
#    power from this storage to the destination zones.
# -----------------------------------------------------------------------------
...
h2_carryover_eff   = fill(1.0,  1, N_HOURS)   # hydrogen standing loss ≈ 0
...
println("  GB_H2 storage: charge=", round(h2_elec_total, digits=1),
        " MW, discharge=", round(h2_electrical_total, digits=1),
        " MW (NESO 120h), energy=", round(H2_ENERGY_CAPACITY_MWH/1000, digits=1), " GWh")

# Append to existing storage arrays from section 4
...
# Add GB_H2 storage to region index
...
# -----------------------------------------------------------------------------
# 6. LINES + INTERFACES -- both buses must be in region_set (now 21 regions
#    including GB_H2). Electrolysis links (zone -> GB_H2) are excluded since
#    that demand is already modelled as fixed load on the zone. H2 turbine
#    links (GB_H2 -> zone) now pass the filter and become PRAS Lines.
#    External HVDC interconnectors are dropped as before.
# -----------------------------------------------------------------------------
...
#   H2 turbine (GB_H2→zone): after swap → forward=0, backward=electrical output
#   Electrolysis (zone→GB_H2): no swap  → forward=p_nom (charging), backward=0
#   AC/DC: symmetric = p_nom
